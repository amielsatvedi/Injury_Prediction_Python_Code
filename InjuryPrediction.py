...
y_pred_test = reg_model13.predict(X_test)
y_pred_train = reg_model13.predict(X_train)
...
#outputs of crossing validation and average scores
print('Cross Validation score: {}'.format(cv_score3))
print("\nAverage 10-Fold Score: {}".format(np.mean(cv_score3)))
...
b_pred_test = reg_model14.predict(A_test)
b_pred_train = reg_model14.predict(A_train)

rmse_test = np.sqrt(mean_squared_error(b_test, b_pred_test))
rmse_train = np.sqrt(mean_squared_error(b_train, b_pred_train))
...
#training the algorithm and outputting the summary statistics
A_train_sm = sm.add_constant(A_train)
sm_model = sm.OLS(b_train, A_train_sm).fit()
print(sm_model.summary())
print(sm_model.rsquared_adj)
